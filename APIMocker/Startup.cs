...
namespace APIMocker
{
    public class Startup
    {
        ...
        private static async Task Respond(HttpResponse response, APIDetailOptions match)
        {
          ...
          if (!string.IsNullOrWhiteSpace(match.ResponseBody))
          {
            await stream.WriteAsync(Encoding.UTF8.GetBytes(match.ResponseBody));
          }
          else if (!string.IsNullOrWhiteSpace(match.ResponseBodyFile))
          {
            var bytes = await File.ReadAllBytesAsync(match.ResponseBodyFile);
